...
class CNN_ENCODER(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = nn.functional.interpolate(x,size=(256, 128), mode='bilinear', align_corners=False)
        # 299 x 299 x 3
        ...
        if features is not None:
            features = self.emb_features(features)  # feature->  17 x 17 x 768  --> 17 x 17 x 256
...
class INIT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(nz, ngf * 4 * 2 * 2, bias=False),
...
class D_GET_LOGITS(nn.Module):
    def __init__(self, ndf, nef, bcondition=False):
        ...
        self.outlogits = nn.Sequential(
            nn.Conv2d(ndf * 8, 1, kernel_size=(4, 2), stride=4),
            nn.Sigmoid())
