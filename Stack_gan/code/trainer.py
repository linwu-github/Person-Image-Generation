...
def define_optimizers(netG, netsD):
    ...
    for i in range(num_Ds):
        ...
        # opt = optim.RMSprop(netsD[i].parameters(), lr=cfg.TRAIN.DISCRIMINATOR_LR, alpha=0.99)
        # opt = optim.SGD(netsD[i].parameters(), lr=cfg.TRAIN.DISCRIMINATOR_LR, momentum=0.9)
...
def save_model(netG, netsD, epoch, model_dir):
...
def save_img_results(imgs_tcpu, fake_imgs, num_imgs, count, image_dir):
    ...
    real_img = imgs_tcpu[-1][0:num]             # 保存原始尺寸的real_img
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            if (epoch+1) % 50 == 0:
                self.d_lr /= 2
                self.g_lr /= 2
                set_lrs(self.optimizersD, self.d_lr)
                set_lr(self.optimizerG, self.g_lr)
        ...
        save_model(self.netG, self.netsD, count, self.model_dir)
...
class condGANTrainer(object):
    ...
    def prepare_data(self, data):
        imgs, w_imgs, t_embedding, _, _ = data
    ...
    def train_Dnet_lz(self, idx, count):
        flag = count % 200
        batch_size = self.real_imgs[0].size(0)
        ...
        if len(real_logits) > 1 and cfg.TRAIN.COEFF.UNCOND_LOSS > 0:
            ...
            if flag==0:
                print('--' * 10)
                print('uncon_loss:', (errD_fake_uncond+errD_wrong_uncond+errD_real_uncond).item())
                print('cond_loss:', (errD_real+errD_fake+errD_wrong).item())
    ...
    def train_Gnet_lz(self, count):
        ...
        mu_1, mu_2 = torch.split(mu, batch_size, 0)
        ...
        for i in range(self.num_Ds):
            ...
            lz_loss_all = lz_loss_all + lz_loss

            ## reconstruction loss
            # loss_G_L1 = self.criterionL1(fake_imgs1, self.real_imgs[i]) * 10
            ...
            if len(outputs1) > 1 and cfg.TRAIN.COEFF.UNCOND_LOSS > 0:
                ...
                if flag==0:
                    print('--'*10)
                    print('uncod_G:', errG_patch.item())
                    print('con_G:', errG.item())
                    print('lz_loss', lz_loss.item())
                    # print('loss_G_L1', loss_G_L1.item())
                errG = errG + errG_patch + lz_loss
            errG_total = errG_total + errG
            # mode-seeking loss
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                #     for i in range(self.num_Ds):
                #         errD = self.train_Dnet(i, count)
                #         errD_total += errD.mean()

                for i in range(self.num_Ds):
                    errD = self.train_Dnet_lz(i, count)
                    errD_total += errD.mean()
                ...
                kl_loss, errG_total, _ = self.train_Gnet_lz(count)
                errG_all +=errG_total.mean()
            ...
            errD_mean = errD_all / (len(self.data_loader))
            errG_mean = errG_all / len(self.data_loader)
            end_t = time.time()
            print('''[%d/%d][%d]  Loss_D: %.6f Loss_G: %.6f Loss_KL: %.2f lr_g: %.8f lr_d: %.8f Time: %.2fs
                                  '''
                  % (epoch, self.max_epoch, self.num_batches, errD_mean,
                      errG_mean,
                     kl_loss.item(), self.g_lr, self.d_lr, end_t - start_t))
            if epoch % 50 == 0:
                save_model(self.netG, self.netsD, epoch, self.model_dir)
