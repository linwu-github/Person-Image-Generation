...
class INIT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_dim, ngf * 4 * 2 * 2, bias=False),
            nn.BatchNorm1d(ngf * 4 * 2 * 2),
            GLU())
    ...
    def forward(self, z_code, c_code=None):
        ...
        out_code = out_code.view(-1, self.gf_dim, 4, 2)
...
class D_NET64(nn.Module):
    ...
    def define_module(self):
        ...
        self.logits = nn.Sequential(nn.Conv2d(ndf * 8, 1, kernel_size=(4, 2), stride=4), nn.Sigmoid())

        if cfg.GAN.B_CONDITION:
            self.jointConv = Block3x3_leakRelu(ndf * 8 + efg, ndf * 8)
            self.uncond_logits = nn.Sequential(nn.Conv2d(ndf * 8, 1, kernel_size=(4, 2), stride=4), nn.Sigmoid())
    ...
    def forward(self, x_var, c_code=None):
        ...
        if cfg.GAN.B_CONDITION and c_code is not None:
            c_code = c_code.view(-1, self.ef_dim, 1, 1)
            c_code = c_code.repeat(1, 1, 4, 2)
            # state size (ngf+egf) x 4 x 4
            h_c_code = torch.cat((x_code, c_code), 1)
            # state size ngf x in_size x in_size
...
class D_NET128(nn.Module):
    ...
    def define_module(self):
        ...
        self.logits = nn.Sequential(nn.Conv2d(ndf * 8, 1, kernel_size=(4,2), stride=1), nn.Sigmoid())

        if cfg.GAN.B_CONDITION:
            self.jointConv = Block3x3_leakRelu(ndf * 8 + efg, ndf * 8)
            self.uncond_logits = nn.Sequential(nn.Conv2d(ndf * 8, 1, kernel_size=(4, 2), stride=1), nn.Sigmoid())
...
class D_NET256(nn.Module):
    ...
    def define_module(self):
        ...
        self.logits = nn.Sequential(
            nn.Conv2d(ndf * 8, 1, kernel_size=(4, 2), stride=1),
            nn.Sigmoid())
