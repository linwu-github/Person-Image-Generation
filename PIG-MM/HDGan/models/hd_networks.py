...
class ImageDown(torch.nn.Module):
    ...
    def __init__(self, input_size, num_chan, out_dim):
        ...
        if input_size == 64:
            ...
            _layers += [conv_norm(cur_dim*4, out_dim,  norm_layer, stride=1, activation=activ, kernel_size=(5, 3), padding=0)]  # 4
        ...
        if input_size == 128:
            ...
            _layers += [conv_norm(cur_dim*8, out_dim,  norm_layer, stride=1, activation=activ, kernel_size=(5, 3), padding=0)]  # 4
...
class Generator(nn.Module):
    def __init__(self, sent_dim, noise_dim, emb_dim, hid_dim, num_resblock=1, side_output_at=[64, 128, 256]):
        ...
        self.vec_to_tensor = Sent2FeatMap(
            emb_dim+noise_dim, 4, 2, self.hid_dim*8)
    ...
    def forward(self, sent_embeddings, z):
        ...
        # id_random, mean_id, logsigma_id = self.condEmbedding_ID(id_embeddings)
        # sent_random = torch.cat([sent_random_1, id_random], dim=1)
        # concat
        ...
        fake_imgs = []
        fake_imgs.append(output_64)
        fake_imgs.append(output_128)
        fake_imgs.append(output_256)

        return fake_imgs, mean, logsigma
...
class Discriminator(torch.nn.Module):
    def __init__(self, num_chan,  hid_dim, sent_dim, emb_dim, side_output_at=[64, 128, 256]):
        ...
        if 64 in side_output_at:  # discriminator for 64 input
            self.img_encoder_64 = ImageDown(64,  num_chan,  enc_dim)  # 4x2
            self.pair_disc_64 = DiscClassifier(enc_dim, emb_dim, kernel_size=(4, 2))
            self.local_img_disc_64 = nn.Conv2d(enc_dim, 1, kernel_size=(4, 2), padding=0, bias=True)
        ...
        if 128 in side_output_at:  # discriminator for 128 input
            self.img_encoder_128 = ImageDown(128,  num_chan, enc_dim)  # 4x2
            self.pair_disc_128 = DiscClassifier(
                enc_dim, emb_dim, kernel_size=(4, 2))
            self.local_img_disc_128 = nn.Conv2d(enc_dim, 1, kernel_size=(4, 2), padding=0, bias=True)
        ...
        if 256 in side_output_at:  # discriminator for 256 input
            ...
            self.pair_disc_256 = DiscClassifier(enc_dim, emb_dim, kernel_size=(4, 2))
            self.pre_encode = conv_norm(enc_dim, enc_dim, norm_layer, stride=1, activation=activ, kernel_size=(5, 3), padding=0)
            # never use 1x1 convolutions as the image disc classifier
            self.local_img_disc_256 = nn.Conv2d(enc_dim, 1, kernel_size=(4, 2), padding=0, bias=True)
            # map sentence to a code of length emb_dim
            _layers = [nn.Linear(sent_dim , emb_dim), activ]
    ...
    def forward(self, images, sent_embedding):
        '''
        Parameters:
        -----------
        images:    (B, C, H, W)
        ...
        # embedding = torch.cat([sent_embedding, id_embeddings], dim=1)
        sent_code = context_emb_pipe(sent_embedding)
